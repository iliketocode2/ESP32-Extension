...
export function activate(context: vscode.ExtensionContext) {
    context.subscriptions.push(
        vscode.commands.registerCommand('spike-serial.open', () => {
            if (panel) {
                panel.reveal();
                return;
            }
            ...
            panel.webview.onDidReceiveMessage(async (msg: { type: string; url?: string }) => {
                if (msg.type === 'openExternal' && msg.url) {
                    ...
                    const choice = await vscode.window.showInformationMessage(
                        'To run locally: navigate to your project folder in a terminal and run `python -m http.server 8000`, then point the URL bar to http://localhost:8000',
                        'Open Terminal'
                    );
